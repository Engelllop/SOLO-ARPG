...
#include "ASOLOFloorManager.generated.h"

class USOLOFloorDataAsset;
...
class SOLOCORE_API ASOLOFloorManager : public AActor
{
	GENERATED_BODY()
...
public:
	...
	TArray<TSoftObjectPtr<USOLOFloorDataAsset>> Floors;
	...
	/** Genera enemigos segÃºn Min/Max, EnemyTypes y BossClass opcional. */
	UFUNCTION(BlueprintCallable, Category = "Tower")
	void SpawnEnemiesForFloor(USOLOFloorDataAsset* FloorData);

	/** Se dispara cuando todos los enemigos generados por el Ãºltimo SpawnEnemiesForFloor han muerto o sido destruidos. */
	UPROPERTY(BlueprintAssignable, Category = "Tower|Events")
	FOnFloorCleared OnFloorCleared;

protected:
	/** Resuelve el DataAsset para un FloorIndex (carga sÃ­ncrona si hace falta). */
	USOLOFloorDataAsset* FindFloorData(int32 FloorIndex);

	void RegisterSpawnedEnemy(ASOLOEnemyCharacter* Enemy);

	UFUNCTION()
	void OnTrackedEnemyDestroyed(AActor* DestroyedActor);

	UPROPERTY(Transient)
	TArray<TWeakObjectPtr<ASOLOEnemyCharacter>> TrackedEnemies;
