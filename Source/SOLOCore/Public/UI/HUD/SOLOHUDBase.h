...
#include "SOLOHUDBase.generated.h"
...
class SOLOCORE_API ASOLOHUDBase : public AHUD
{
	GENERATED_BODY()
...
public:
	...
	void ShowBossHealthBar(ASOLOEnemyCharacter* Boss);
	...
	/**
	 * Muestra un nÃºmero de daÃ±o del pool: proyecciÃ³n inicial a pantalla, float-up/fade en el widget (1,5s por defecto).
	 */
	UFUNCTION(BlueprintCallable, Category = "HUD|Damage")
...
private:
	...
	TWeakObjectPtr<ASOLOEnemyCharacter> CachedBossEnemy;
	TWeakObjectPtr<UAbilitySystemComponent> CachedBossASC;

	/** Inventario al que se enlazÃ³ OnCurrencyChanged hacia el overlay (desenlazar en EndPlay). */
	UPROPERTY()
	TObjectPtr<USOLOInventoryComponent> OverlayReactiveInventory;

	/** QuestComponent enlazado al overlay para el tracker. */
	UPROPERTY()
	TObjectPtr<USOLOQuestComponent> OverlayReactiveQuest;
