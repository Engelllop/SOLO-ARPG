...
struct SOLOCORE_API FMinimapDot
{
	GENERATED_BODY()

	// Normalized [-1,1] space relative to player â€” Blueprint converts to widget coords
	...
	// Weak ref â€” actor may be destroyed between ticks
	UPROPERTY(BlueprintReadWrite, Category = "MinimapDot")
	TObjectPtr<AActor> Owner = nullptr;
};

UCLASS(BlueprintType, Blueprintable, meta = (DisableNativeTick = "false"))
class SOLOCORE_API UWG_Minimap : public UUserWidget
{
	GENERATED_BODY()
...
private:
	...
	// Builds ActiveDots from the world â€” called every tick
