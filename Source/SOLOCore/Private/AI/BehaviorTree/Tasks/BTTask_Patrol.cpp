...
bool UBTTask_Patrol::TryPickPatrolPoint(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	const FVector Origin     = BB ? BB->GetValueAsVector(ASOLOAIController::BB_PatrolOrigin) : FVector::ZeroVector;
...
void UBTTask_Patrol::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	{
		// Arrived â€” start the idle wait before picking the next point
		Mem->bWaiting    = true;
		Mem->WaitElapsed = 0.f;
	}
}
