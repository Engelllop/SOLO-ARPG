...
void UBTService_RotateToTarget::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	// Only rotate while aggressive â€” during patrol it looks weird to face a ghost target
	if (!BB->GetValueAsBool(ASOLOAIController::BB_bIsAggro))
	{
		return;
	}
	...
	// Interpolate Yaw only â€” we never want the enemy to tilt pitch toward the player
