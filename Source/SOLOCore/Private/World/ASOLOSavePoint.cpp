...
#define LOCTEXT_NAMESPACE "SOLOSavePoint"

ASOLOSavePoint::ASOLOSavePoint()
{
...
void ASOLOSavePoint::Interact_Implementation(ASOLOPlayerCharacter* Interactor)
{
	if (!Interactor)
	{
		return;
...
void ASOLOSavePoint::ServerPerformSave_Implementation(ASOLOPlayerCharacter* Interactor)
{
	PerformSaveInteraction(Interactor);
}

void ASOLOSavePoint::PerformSaveInteraction(ASOLOPlayerCharacter* Interactor)
{
	if (!Interactor)
	{
		return;
	...
	{
		if (ASOLOGameMode* GM = World->GetAuthGameMode<ASOLOGameMode>())
		{
...
void ASOLOSavePoint::MulticastPlaySaveEffects_Implementation()
{
...
FText ASOLOSavePoint::GetInteractPrompt_Implementation() const
{
	return LOCTEXT("SavePrompt", "Guardar partida");
}

bool ASOLOSavePoint::CanInteract_Implementation(ASOLOPlayerCharacter* Interactor) const
{
	if (bSingleUse && bHasBeenUsed)
	{
		return false;
