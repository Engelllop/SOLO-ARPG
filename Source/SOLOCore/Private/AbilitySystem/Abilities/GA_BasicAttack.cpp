...
UGA_BasicAttack::UGA_BasicAttack()
{
	...
	AbilityInputID = ESOLOAbilityInputID::BasicAttack;
...
void UGA_BasicAttack::PlayCurrentComboMontage()
{
	...
	// Open combo window at ~60% through the animation â€” designer tunes montage notify instead in BP
...
void UGA_BasicAttack::ApplyDamageToTarget(AActor* Target)
{
	...
	if (ACharacter* Avatar = Cast<ACharacter>(GetAvatarActorFromActorInfo()))
	{
		if (Avatar->HasAuthority())
		{
			...
			{
				if (USOLOSkillMasteryComponent* Mastery = PS->FindComponentByClass<USOLOSkillMasteryComponent>())
				{
...
{
	// Blend out fires before completed on some montages â€” let OnMontageCompleted handle logic
}
