...
void UWG_MainHUD::NativeConstruct()
{
	...
	CachedPlayerState = PC->GetPlayerState<ASOLOPlayerState>();
	...
	const USOLOAttributeSet* AS = CachedPlayerState->GetAttributeSet();
...
void UWG_MainHUD::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	// VitalBars and Minimap each drive their own Tick â€” nothing to do here centrally
}
...
{
	const USOLOAttributeSet* AS = CachedPlayerState ? CachedPlayerState->GetAttributeSet() : nullptr;
	if (!AS)
	{
		return;
	}
