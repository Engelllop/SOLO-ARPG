...
void UWG_BossHealthBar::SetBossReference(AEMBERVEILEnemyCharacter* Boss)
{
	CachedBoss = Boss;
	if (Boss)
	{
...
void UWG_BossHealthBar::BindToBoss(AEMBERVEILEnemyCharacter* Boss)
{
	if (!Boss) return;
	...
	// Initial health
	if (const UEMBERVEILAttributeSet* AS = Boss->GetAttributeSet())
	{
	...
	ASC->GetGameplayAttributeValueChangeDelegate(UEMBERVEILAttributeSet::GetHealthAttribute())
...
void UWG_BossHealthBar::OnHealthChanged(const FOnAttributeChangeData& Data)
{
	...
	{
		if (const UEMBERVEILAttributeSet* AS = CachedBoss->GetAttributeSet())
		{
			const float MaxH = FMath::Max(AS->GetMaxHealth(), 1.f);
			UpdateHealthPercent(AS->GetHealth() / MaxH);
		}
...
void UWG_BossHealthBar::InitBoss(const FText& InBossName, int32 InTotalPhases)
{
	...
	// E.g. 3 phases → thresholds at 0.667 and 0.333
...
void UWG_BossHealthBar::SetVisible(bool bVisible, float FadeDuration)
{
	...
	// Collapse after fade completes — Blueprint calls SetVisibility(Collapsed) at animation end
}
