...
UGA_Dodge::UGA_Dodge()
{
	...
	AbilityInputID = EEMBERVEILAbilityInputID::Dodge;
	...
	DodgeInvulnerabilityEffect = UEMBERVEIL_GE_DodgeInvulnerability::StaticClass();
}
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	// i-frames: GameplayEffect con State.Invulnerable (replicado por el ASC en servidor/clientes)
	{
		const TSubclassOf<UGameplayEffect> InvulnClass = DodgeInvulnerabilityEffect
			...
			: TSubclassOf<UGameplayEffect>(UEMBERVEIL_GE_DodgeInvulnerability::StaticClass());
		if (InvulnClass)
		{
	...
	// Estado de animación / lógica de juego (no confundir con invulnerabilidad)
	{
	...
	// Montaje: UGameplayAbility::PlayMontageAndWait → ASC->PlayMontage (FGameplayAbilityRepAnimMontage replicado a todos los clientes).
	if (DodgeMontage)
	{
	...
	// Desplazamiento: Root Motion Source constante (no solo animación: empuje físico vía CMC; el movimiento se replica).
