...
#include "EMBERVEILItemTypes.generated.h"
...
/**
 * Categoría funcional del item. Determina cómo se usa y dónde puede ir en el inventario.
 */
UENUM(BlueprintType)
enum class EEMBERVEILItemType : uint8
{
 ...
 */
UENUM(BlueprintType)
enum class EEMBERVEILItemRarity : uint8
{
 ...
 */
UENUM(BlueprintType)
enum class EEMBERVEILEquipmentSlot : uint8
{
...
struct EMBERVEILCORE_API FItemStatModifier
{
    GENERATED_BODY()

    /** Atributo de gameplay que se modifica (p.ej. UEMBERVEILAttributeSet::GetStrengthAttribute()) */
 ...
 * Usado tanto en la lógica del componente como en la UI.
 */
USTRUCT(BlueprintType)
struct EMBERVEILCORE_API FInventorySlot
{
    GENERATED_BODY()

    /** Instancia runtime del item (nula si el slot está vacío) */
    UPROPERTY(BlueprintReadOnly, Category = "Inventory Slot")
    TObjectPtr<UEMBERVEILItemInstance> ItemInstance;

    /** Cantidad actual en este slot */
    UPROPERTY(BlueprintReadOnly, Category = "Inventory Slot")
    int32 Quantity = 0;

    /** Devuelve true si el slot no tiene ningún item asignado */
    bool IsEmpty() const { return ItemInstance == nullptr || Quantity <= 0; }
};

/** Par slot + instancia para replicación (TMap no es compatible con UHT + Replicated). */
USTRUCT(BlueprintType)
struct EMBERVEILCORE_API FReplicatedEquipmentEntry
{
    GENERATED_BODY()

    UPROPERTY(BlueprintReadOnly, Category = "Equipment")
    EEMBERVEILEquipmentSlot Slot = EEMBERVEILEquipmentSlot::None;

    UPROPERTY(BlueprintReadOnly, Category = "Equipment")
    TObjectPtr<UEMBERVEILItemInstance> ItemInstance = nullptr;
};
